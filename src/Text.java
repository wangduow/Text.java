...
public class Text {//主程序
	public static void main(String[] args){
		...
		Student[] stuArr=new Student[3];//声明并实例化对象
		Admin adminStu=new Admin();//声明并实例化对象
		while(true){
			...
			if(number.equals("10")){
				...
				System.out.println("请输入学生的编号");
				int on=in.nextInt();
				adminStu.input(on,stuArr);//调用对象adminStu的方法input
				...
				System.out.println("执行根据总分进行排序操作");
				int num=in.nextInt();
			...
			//for(int i=0;i<stuArr.length;i++){//输出学生
//				if(stuArr[i]!=null){//数组空值不输出
...
//			}
		...
		//		stu.setAvg();
		//		System.out.println("\t编号\t姓名\t年龄\tjava\tc#\thtml\tsql\tsum\tavg");
