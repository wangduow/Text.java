...
public class Admin {//用来管理学生的一个类
	String msg="\t编号\t姓名\t年龄\tjava\tc#\thtml\tsql\tsum\tavg";
	public void print(Student[] arr){//更新数据的
		System.out.println(msg);
		for(int i=0;i<arr.length;i++){
			if(arr[i]!=null){
				arr[i].setSum();
				arr[i].setAvg();
				System.out.println(arr[i]);//将对象输出
			}
		}
	}	
	public boolean exist(int on,Student stu){//判断学生是否存在
		if(stu!=null){
			if(stu.getOn()==on){//判断编号是不是等于输入的编号
				return true;
			}
			else{
				return false;
	...
	public void create(String name,int age,Student[] arr){//添加学生的方法
		...
		stu.setName(name);
		stu.setAge(age);
		int i=this.setIndex(arr);//调用方法得到int型的指针i
		stu.setOn(i);
		if(i==99999){
			System.out.println("人数已满，不能再添加");
		}else{
			arr[i]=stu;
		}
		this.print(arr);//更新
	}
	public int setIndex(Student[] arr){//返回数组为空的下标，便于添加人数
		for(int i=0;i<arr.length;i++){
			if(arr[i]==null){
				return i;
			}
	...
	public void update(int on,String name,int age,Student[] arr){//更新学员信息
		for(int i=0;i<arr.length;i++){
			if(this.exist(on,arr[i])){//this表示当前方法，判断编号为on和索引为i的数组是否存在
				arr[i].setName(name);
				arr[i].setAge(age);
